...
const NewBoard = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (
        isError(error) &&
        error.message === "Column names must be unique within the same board"
      ) {
        alert("Column names must be unique within the same board");
      } else {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="form new-board-form">
      ...
      <div className="form-inputs-container">
        ...
        <div className="mar-b">
          ...
          />
          {errors?.boardName && (
            <span className="error">{errors?.boardName?.message}</span>
          )}
        </div>
